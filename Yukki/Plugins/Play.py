...
async def play(_, message: Message):
    ...
    if audio:
        mystic = await message.reply_text(
            "🔄 Memproses Audio...Mohon Menunggu!"
        )
        try:
            read = db_mem[message.chat.id]["live_check"]
            if read:
                return await mystic.edit("Pemutaran Live Streaming... Hentikan untuk memutar musik")
            else:
                pass
        except:
            pass 
        ...
        return await message.reply_photo(
            photo=thumb,
            caption=f"📎Judul: **{title}\n\n⏳Durasi:** {duration_min} Mins\n\nTekan Salah satu Tombol 🎵/🎥 untuk memutar Lagu\n\n__[Dapatkan Informasi Tambahan Tentang Video](https://t.me/{BOT_USERNAME}?mstart=info_{videoid})__",
        ...
        if len(message.command) < 2:
            ...
            await message.reply_photo(
                ...
                caption=(
                    "**Usage:** /play [Nama Musik atau Tautan Youtube atau Balas Audio]\in\Jika Anda ingin memutar Daftar Putar! Pilih salah satu dari bawah."
                ),
                reply_markup=InlineKeyboardMarkup(buttons),
            )
            return
